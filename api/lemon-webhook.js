export default async function handler(req, res) {
  ...
  try {
    ...
    if (!storeId || !productId || !apiKey) {
      console.error('Missing Lemon Squeezy config:', { storeId, productId, hasApiKey: !!apiKey })
      throw new Error('Missing Lemon Squeezy configuration')
    }

    console.log('Creating checkout for:', { userId, email, productId })
      ...
      body: JSON.stringify({
        data: {
          ...
          relationships: {
            ...
            // Utiliser le product directement sans variant
            product: {
              data: {
                type: 'products',
